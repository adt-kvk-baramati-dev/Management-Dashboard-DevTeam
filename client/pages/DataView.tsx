...
export default function DataView() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError(
        "Network error fetching data. Make sure API and MongoDB credentials are set.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const downloadExcel = () => {
    ...
    const filename = searchPrn
      ? `PRN_${searchPrn}_Data_${dateStr}.xlsx`
      : `Complete_Data_${dateStr}.xlsx`;
  ...
  const renderValue = (
    val: any,
    className = "text-emerald-700 bg-emerald-50 font-medium",
  ) => {
    if (val === null || val === undefined || val === "")
      return (
        <td className="p-3 border-b text-slate-400 italic bg-slate-50">0</td>
      );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-[1600px] mx-auto">
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 bg-[#e8eee2] p-6 rounded-2xl shadow-sm border border-slate-200">
          <div>
            ...
            <p className="text-slate-500 mt-1 pl-11">
              Consolidated view of all deployed sensor & manual data.
            </p>
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            ...
            <Button onClick={downloadExcel} className="whitespace-nowrap">
        ...
        <div className="bg-[#e8eee2] rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50">
            <div className="relative w-full max-w-sm">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-shadow bg-[#e8eee2]"
              />
              {searchPrn && (
                <button
            ...
            <div className="text-sm font-medium text-slate-500 bg-[#e8eee2] px-4 py-2 rounded-full border border-slate-200 shadow-sm">
          ...
          <div className="overflow-x-auto w-full relative h-[70vh] custom-scrollbar">
            {loading ? (
              ...
              <table
                ...
                className="w-full text-sm text-left whitespace-nowrap border-collapse"
              >
                <thead className="text-xs uppercase bg-slate-800 text-white sticky top-0 z-20 shadow-md">
                  <tr>
                    <th
                      rowSpan={2}
                      className="px-4 py-3 border-r border-slate-700 bg-slate-900 sticky left-0 z-30 min-w-[120px]"
                    >
                      📅 Date
                    </th>
                    <th
                      rowSpan={2}
                      className="px-4 py-3 border-r border-slate-700 bg-slate-900 sticky left-[120px] z-30 min-w-[100px]"
                    >
                      🔢 PRN
                    </th>
                    <th
                      rowSpan={2}
                      className="px-4 py-3 border-r border-slate-700 bg-slate-900 sticky left-[220px] z-30 min-w-[150px]"
                    >
                      👤 Farmer Name
                    </th>
                    <th
                      colSpan={19}
                      className="px-4 py-2 border-r border-slate-700 text-center bg-blue-900/60 border-b border-blue-800"
                    >
                      🌾 MapMyCrop Data
                    </th>
                    <th
                      colSpan={18}
                      className="px-4 py-2 border-r border-slate-700 text-center bg-emerald-900/60 border-b border-emerald-800"
                    >
                      🌡️ Fasal History Data
                    </th>
                    <th
                      colSpan={4}
                      className="px-4 py-2 text-center bg-amber-900/60 border-b border-amber-800"
                    >
                      🧪 KVK Data
                    </th>
                  </tr>
                  <tr>
                    <th className="px-3 py-2 border-r border-slate-700 bg-blue-900/40">
                      ID
                    </th>
                    <th className="px-3 py-2 border-r border-slate-700 bg-blue-900/40">
                      FarmID
                    ...
                    <th className="px-3 py-2 border-r border-slate-700 bg-emerald-900/40">
                      Plot
                    </th>
                    <th className="px-3 py-2 border-r border-slate-700 bg-emerald-900/40">
                      Farm
                    ...
                    <th className="px-3 py-2 border-r border-slate-700 bg-amber-900/40">
                      Week
                    </th>
                    <th className="px-3 py-2 border-r border-slate-700 bg-amber-900/40">
                      pH
                ...
                <tbody className="bg-[#e8eee2] divide-y divide-slate-100">
                  {filteredData.map((row, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-3 font-medium text-slate-700 bg-slate-50 sticky left-0 z-10 border-r border-slate-200">
                          {row.date}
                        </td>
                        <td className="px-4 py-3 font-bold text-emerald-600 bg-slate-50 sticky left-[120px] z-10 border-r border-slate-200">
                          #{row.prn_no}
                        </td>
                        <td className="px-4 py-3 font-medium text-slate-800 bg-slate-50 sticky left-[220px] z-10 border-r border-slate-200 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">
                          {row.farmer_name}
                        </td>
                        ...
                        <td className="p-3 border-b text-slate-400 italic bg-amber-50">
                          -
                        </td>
                      </tr>
                    );
